...
def leOperacoes(nomeArqOP: str):
    ...
    while linha:
        ...
        if char[0] == "b":
            ...
            if b == True:
                offset = calcOffset(Identificador, arqG)
                print(reg.strip())
    ...
    if offset == -1: #SE A CABECA DA LED FOR -1 ELE ENTRA DIRETO NESSA FUNACAO E ADICIONA O REGISTRO NO FIM DO ARQUIVO
        ...
        if tam > tamOffset: #SE O TAMANHO DO REGISTRO NOVO A SER INSERIDO FOR MAIOR QUE O TAMANHO DISPONIVEL SO INSERIR NO FIM
            ...
            if tamOffset > tam:
                ...
                tamSobraExibicao = tamOffset-tam-2
                if tamSobraExibicao == -1:
                    print(f"Tamanho reutilizado {tamOffset}. (Sobra de 1 bytes)")
                else:
                ...
                if tamSobraExibicao+2 >= tamMinimoSobra:#TEM QUE ADICIONAR A SOBRA NO ROLE#### #+2 para ser o tamanho bruto da sobra, sem o debito dos campos de tamanho
                    tamSobraExibicao = tamSobraExibicao.to_bytes(2) #tamOffset agora é o tamanho do novo registro (sobra)
                    registro = "*"
                    registro = registro.encode()
                    teste = -1
                    teste = teste.to_bytes(4, byteorder='big', signed=True)
                    tudo_junto = tamSobraExibicao + registro + teste
                    arq.seek(posicaoPontero)
                    arq.write(tudo_junto)
                    arq.close()
                    tamSobraExibicao = int.from_bytes(tamSobraExibicao, byteorder='big', signed=False)
                    adicionar_na_led(posicaoPontero, tamSobraExibicao)
                else:
